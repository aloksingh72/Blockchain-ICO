...
interface ERC20{
    ...
    function balance(address amount ) external view returns(uint256);
    
    function allowance(address owner ,address spender) external view returns(uint256);
    
    function approve(address spender,uint256 amount) external returns(bool);
    function tranferForm(address sender, address recipient ,uint256 amount) external returns(bool);
...
contract TokenICO {
    ...
    modifier onlyOwner(){
        require(msg.sender == owner,"Only contract owner can perfrom this action" );
        _;
    ...
    function buyToken(uint256 _tokenAmount)public payable{
        require(msg.value == multiply(_tokenAmount, tokenSalePrice),"Insufficient Ehter provided for the token purchase");
    ...
    payable(owner).jtransfer(msg.value);
    ...
    function tranferEther(address payable _receiver,uint256 _amount) external payable{
           require(msg.value >= _amount,"Insufficient funds sent");
